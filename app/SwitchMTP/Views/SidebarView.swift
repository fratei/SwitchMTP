...
struct SidebarView: View {
    ...
    /// A storage row shows a name and a capacity bar, but nothing about whether
    /// it will accept writes. Without this, the first sign that a storage is
    /// read-only is the Import button greying out several clicks later.
    private func storageTooltip(_ storage: MTPStorage) -> String {
        if storage.virtual {
            return String(localized: "\(storage.name) — contents are generated by DBI on demand and cannot be changed.", comment: "Tooltip for a virtual storage in the sidebar")
        }
        if storage.capabilities.write {
            return String(localized: "\(storage.name) — you can browse, download and upload here.", comment: "Tooltip for a writable storage in the sidebar")
        }
        return String(localized: "\(storage.name) — read-only. You can browse and download, but not change anything.", comment: "Tooltip for a read-only storage in the sidebar")
    }
...
private struct InstallStorageDropZone: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            .buttonStyle(.plain)
            .help(String(localized: "Click to choose files, or drag them here. Titles are queued and sent one at a time; DBI installs them on the console.", comment: "Tooltip for an install drop target in the sidebar"))
