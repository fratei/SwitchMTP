...
struct FileListView: View {
    ...
    var body: some View {
        ...
        .alert(String(localized: "Rename"), isPresented: $isShowingRenameDialog) {
            ...
            Button(String(localized: "Rename")) {
                if canRenameInSelectedStorage, let file = fileToRename {
        ...
        .alert(String(localized: "Replace and merge the existing items?"), isPresented: $isShowingReplaceAlert) {
            Button(String(localized: "Replace")) {
                ...
                guard canUploadToSelectedStorage else {
                    errorMessage = String(localized: "This storage does not allow uploads.")
                    showError = true
    ...
    private func handleImportRequest(_ droppedURLs: [URL]) {
        if droppedURLs.isEmpty {
            errorMessage = String(localized: "No files were provided.")
            showError = true
            return
        }
        guard canUploadToSelectedStorage else {
        ...
        ZStack {
            FileListTableRepresentable(
                ...
                onDeleteSelected: { _ in
                    NotificationCenter.default.post(name: NSNotification.Name("SwitchMTPDeleteAction"), object: nil)
                },
...
private extension FileListView {
    var selectedStorage: MTPStorage? { manager.selectedStorage }
    var canUploadToSelectedStorage: Bool { selectedStorage?.capabilities.write == true && selectedStorage?.virtual != true }
    var canCreateFolderInSelectedStorage: Bool { selectedStorage?.capabilities.makeDirectory == true && selectedStorage?.virtual != true }
    var canRenameInSelectedStorage: Bool { selectedStorage?.capabilities.rename == true && selectedStorage?.virtual != true }
    var canDeleteInSelectedStorage: Bool { selectedStorage?.capabilities.delete == true && selectedStorage?.virtual != true }
...
private struct FileListTableRepresentable: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTableViewDataSource, NSTableViewDelegate, NSFilePromiseProviderDelegate, QLPreviewPanelDataSource, QLPreviewPanelDelegate {
        ...
        init(parent: FileListTableRepresentable) {
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(handleOpenActionFromNotification), name: NSNotification.Name("SwitchMTPOpenFileAction"), object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(handleQuickLookActionFromNotification), name: NSNotification.Name("SwitchMTPToggleQuickLook"), object: nil)
        }

        private var selectedStorage: MTPStorage? { parent.manager.selectedStorage }
        private var canUpload: Bool { selectedStorage?.capabilities.write == true && selectedStorage?.virtual != true }
        private var canCreateFolder: Bool { selectedStorage?.capabilities.makeDirectory == true && selectedStorage?.virtual != true }
        private var canRename: Bool { selectedStorage?.capabilities.rename == true && selectedStorage?.virtual != true }
        private var canDelete: Bool { selectedStorage?.capabilities.delete == true && selectedStorage?.virtual != true }
        ...
        func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
            ...
            case .size:
                let help = file.sizeUnknown ? String(localized: "Size not reported by the device") : nil
                return makeTextCell(
                    identifier: "SizeCell",
        ...
        func menu(for clickedRow: Int) -> NSMenu {
            ...
            if !selectedFiles.isEmpty {
                ...
                if selectedFiles.count == 1, let _ = selectedFiles.first {
                    ...
                    if !canRename {
                        ...
                        renameItem.toolTip = String(localized: "This storage does not allow renaming items.")
                    }
                ...
                if !canDelete {
                    ...
                    deleteItem.toolTip = String(localized: "This storage does not allow deleting items.")
                }
            ...
            if !canCreateFolder {
                ...
                newFolder.toolTip = String(localized: "This storage does not allow creating folders.")
            }
        ...
        private func openFileFromCacheOrDownload(_ file: MTPFile) {
            let tempDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("SwitchMTP_QuickLook", isDirectory: true)
        ...
        func canPaste() -> Bool {
            return canUpload && !readFilesFromPasteboard().isEmpty
        }
        ...
        private func makeTextCell(identifier: String, text: String, alignment: NSTextAlignment, font: NSFont, toolTip: String? = nil, tableView: NSTableView) -> NSView {
        ...
        private func preparePreviewItem(for file: MTPFile) -> URL? {
            let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("SwitchMTP_QuickLook", isDirectory: true)
            do {
        ...
        private static let overlayViewIdentifier = NSUserInterfaceItemIdentifier("SwitchMTP_QLOverlay")
